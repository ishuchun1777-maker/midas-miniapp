...
export function OnboardingSlides({ lang, theme, onFinish }) {
  ...
  const slides = {
    ...
    ru:[
      {icon:"🎯",title:"Что такое MIDAS?",text:"Умная платформа для соединения предпринимателей и рекламодателей. Найти нужного партнёра теперь просто!"},
      ...
      {icon:"📋",title:"Тендерная площадка",text:"Предприниматель публикует бриф, рекламодатели — предложения. Лучшая цена и план побеждает!"},
      {icon:"🤖",title:"AI Консультант",text:"Профессиональные советы на основе реальных данных вашей сферы и платформы."},
    ]
  };

  const sl  = slides[lang];
  const cur = sl[slide];
    ...
    <div style={{minHeight:"100vh",background:theme.bg,padding:"32px 20px",display:"flex",flexDirection:"column",justifyContent:"center"}}>
      {modal && (
        <Modal title={modal==="oferta"?lg.oferta_title:lg.privacy_title} onClose={()=>setModal(null)} theme={theme}>
          <pre style={{fontSize:11,color:theme.sub,whiteSpace:"pre-wrap",lineHeight:1.8,fontFamily:"inherit"}}>{modal==="oferta"?lg.oferta:lg.privacy}</pre>
        </Modal>
      )}
...
export function Registration({ tgUser, onDone, theme }) {
  ...
  const T = {
    uz:{name:"To'liq ismingiz *",phone:"Telefon raqam *",phonePh:"+998901234567",
      phoneNote:"Faqat adminga ko'rinadi",phoneErr:"Format: +998XXXXXXXXX",
      fill:"Barcha * maydonlarni to'ldiring",sector:"Soha tanlang *",
      sub:"Soha turi (bir nechta)",region:"Hudud",save:"Saqlash",saving:"Saqlanmoqda...",
      back:"← Orqaga",next:"Davom etish →",verifyNote:"Ma'lumotlaringiz admin tomonidan tekshiriladi.",
      platform:"Platforma tanlang *",link:"Profil havolasi *",fol:"Obunachi soni *",
      er:"Aktivlik (%)",pPost:"Post narxi (so'm) *",pStory:"Story narxi (so'm)",
      pVideo:"Video narxi (so'm)",pDesc:"Narx haqida qo'shimcha",addr:"Aniq manzil *",
      coords:"Koordinata (ixtiyoriy)"},
    ru:{name:"Полное имя *",phone:"Номер телефона *",phonePh:"+998901234567",
      phoneNote:"Виден только администратору",phoneErr:"Формат: +998XXXXXXXXX",
      fill:"Заполните все поля *",sector:"Выберите сферу *",
      sub:"Вид деятельности (несколько)",region:"Регион",save:"Сохранить",saving:"Сохранение...",
      back:"← Назад",next:"Продолжить →",verifyNote:"Ваши данные проверит администратор.",
      platform:"Выберите платформу *",link:"Ссылка на профиль *",fol:"Кол-во подписчиков *",
      er:"Активность (%)",pPost:"Цена поста (сум) *",pStory:"Цена story (сум)",
      pVideo:"Цена видео (сум)",pDesc:"Доп. о цене",addr:"Точный адрес *",
      coords:"Координаты (необязательно)"}
  }[lang];
    ...
    try {
      ...
      if(role==="tadbirkor") {
        ...
        await h(`/api/reklamachi-profiles/${tgUser.id}`,"POST",{
          ...
          price_video:Number(pVideo)||0, price_description:pDesc,
          address, coordinates:coords,
          audience_ages:[], audience_gender:"all", audience_location:"all", interests:[]
        });
      }
      onDone(lang);
      ...
      {[
        ...
        <button key={rr.r} onClick={()=>{setRole(rr.r);setStep(2);}} style={{background:theme.card,border:`2px solid ${theme.border}`,borderRadius:20,padding:"20px",textAlign:"left",cursor:"pointer",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            ...
            <div><div style={{fontWeight:700,fontSize:17,color:theme.text}}>{rr.title}</div>
            <div style={{fontSize:12,color:theme.sub,marginTop:3}}>{rr.desc}</div></div>
  ...
  if(step===3 && role==="tadbirkor") return (
    <div style={S}>
      ...
      <div style={{maxHeight:280,overflowY:"auto",display:"flex",flexDirection:"column",gap:6,marginBottom:16}}>
        {SECTORS.map(s=>(
          <div key={s.v}>
            <button onClick={()=>{setSector(s.v);setSubSecs([]);setExpSec(expSec===s.v?null:s.v);setErrs(e=>({...e,sector:""}));}} style={{width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 14px",borderRadius:12,border:`1.5px solid ${sector===s.v?theme.accent:theme.border}`,background:sector===s.v?theme.accentLight:theme.card,color:sector===s.v?theme.accent:theme.text,fontWeight:600,fontSize:13,cursor:"pointer",boxSizing:"border-box"}}>
            ...
            {expSec===s.v && s.sub?.length>0 && (
              <div style={{marginLeft:14,marginTop:6,display:"flex",flexWrap:"wrap",gap:6}}>
                {s.sub.map(sub=>(
                  <button key={sub.v} onClick={()=>setSubSecs(p=>p.includes(sub.v)?p.filter(x=>x!==sub.v):[...p,sub.v])} style={{padding:"6px 12px",borderRadius:16,fontSize:12,fontWeight:600,border:`1.5px solid ${subSecs.includes(sub.v)?theme.accent:theme.border}`,background:subSecs.includes(sub.v)?theme.accent:theme.card2,color:subSecs.includes(sub.v)?"#fff":theme.sub,cursor:"pointer"}}>
      ...
      <div style={{marginBottom:10}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
          {PLATFORMS_ONLINE.map(p=>(
            <button key={p.v} onClick={()=>{setPlatform(p.v);setErrs(e=>({...e,platform:""}));}} style={{padding:"14px 10px",borderRadius:14,fontSize:12,fontWeight:700,border:`1.5px solid ${platform===p.v?theme.accent:theme.border}`,background:platform===p.v?theme.accentLight:theme.card,color:platform===p.v?theme.accent:theme.text,cursor:"pointer",textAlign:"center"}}>
              <div style={{fontSize:24,marginBottom:4}}>{p.icon}</div>{p[`l_${lang}`]||p.l_uz}
            </button>
          ))}
        </div>
