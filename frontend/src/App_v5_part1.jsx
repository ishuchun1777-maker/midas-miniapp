// ═══════════════════════════════════════════════════════
// MIDAS v7 — Part 1: Theme · Helpers · UI Atoms · Logo
// ═══════════════════════════════════════════════════════
...
// ── FORMAT UTILITIES ───────────────────────────────────
// Sonni 1 000 000 formatida ko'rsatish
export const fmtNum = n => {
  if (!n && n !== 0) return "—";
  return Number(n).toLocaleString("ru-RU");
};

// Kiritish paytida summa formatlash (real-time)
export const formatMoney = (val) => {
  const digits = val.replace(/\D/g, "");
  return digits.replace(/\B(?=(\d{3})+(?!\d))/g, " ");
};

// Faqat raqam qaytarish (saqlash uchun)
export const parseMoney = (val) => {
  return parseInt(val.replace(/\s/g, ""), 10) || 0;
};

// Sana validatsiyasi: DD.MM.YYYY
...
// Sana formatlash kiritish paytida
...
// DD.MM.YYYY → YYYY-MM-DD (backend uchun)
export const dateToISO = (val) => {
  const m = val.match(/^(\d{2})\.(\d{2})\.(\d{4})$/);
  if (!m) return val;
  return `${m[3]}-${m[2]}-${m[1]}`;
};
...
export const MidasLogo = ({ size = 44, theme, white = false }) => {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 120 120" fill="none">
      ...
      <polygon points="60,5 112,32.5 112,87.5 60,115 8,87.5 8,32.5" fill="url(#mG2)" filter="url(#mShadow)" />
      <polygon points="60,5 112,32.5 112,87.5 60,115 8,87.5 8,32.5" fill="none" stroke="url(#mG1)" strokeWidth="2.5" />
      <polygon points="60,20 97,41 97,79 60,100 23,79 23,41" fill="none" stroke="url(#mG1)" strokeWidth="1.5" opacity="0.55" />
      <path d="M35,38 L35,82 M35,38 L60,66 L85,38 M85,38 L85,82" stroke="url(#mG1)" strokeWidth="9" strokeLinecap="round" strokeLinejoin="round" />
...
export const Btn = ({ children, onClick, disabled, theme, v = "primary", full = false, sz = "md", style = {} }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{
      ...
      transition: "opacity 0.15s, transform 0.1s", letterSpacing: "0.01em", ...style
    }}>{children}</button>
  );
};

// Summa input — real-time formatlash bilan
export const MoneyInp = ({ label, value, onChange, err, note, theme, suffix = "so'm" }) => {
  const handleChange = (raw) => {
    const fmt = formatMoney(raw);
    onChange(fmt);
  };
  return (
      ...
      <div style={{ position: "relative" }}>
        <input
          ...
          style={{
            ...
            borderRadius: 12, border: `1.5px solid ${err ? theme.danger : theme.border}`,
            background: theme.inputBg, color: theme.text,
            fontSize: 13, outline: "none", boxSizing: "border-box", fontFamily: "inherit"
          }}
        />
        <span style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", color: theme.sub, fontSize: 12, fontWeight: 600 }}>{suffix}</span>
      </div>
      {err && <div style={{ color: theme.danger, fontSize: 11, marginTop: 4, fontWeight: 500 }}>⚠ {err}</div>}
      {note && <div style={{ color: theme.hint, fontSize: 11, marginTop: 4 }}>ℹ {note}</div>}
    </div>
  );
};

// Sana input — DD.MM.YYYY formati
export const DateInp = ({ label, value, onChange, err, theme, minToday = false }) => {
  const handleChange = (raw) => {
    const fmt = formatDate(raw);
    onChange(fmt);
  };
  const isValid = value.length === 10 ? validateDate(value) : true;
  const showErr = value.length === 10 && !isValid;
  return (
    <div style={{ marginBottom: 14 }}>
      {label && <div style={{ fontSize: 12, color: theme.sub, marginBottom: 5, fontWeight: 600 }}>{label}</div>}
      <div style={{ position: "relative" }}>
        <input
          ...
          style={{
            ...
            borderRadius: 12,
            border: `1.5px solid ${(err || showErr) ? theme.danger : theme.border}`,
            background: theme.inputBg, color: theme.text,
            fontSize: 13, outline: "none", boxSizing: "border-box", fontFamily: "inherit"
          }}
        />
      ...
      {showErr && <div style={{ color: theme.danger, fontSize: 11, marginTop: 4, fontWeight: 500 }}>⚠ Noto'g'ri sana. Masalan: 31.12.2025</div>}
      {err && !showErr && <div style={{ color: theme.danger, fontSize: 11, marginTop: 4, fontWeight: 500 }}>⚠ {err}</div>}
      <div style={{ color: theme.hint, fontSize: 11, marginTop: 4 }}>Kun.Oy.Yil — masalan: 25.12.2025</div>
    </div>
  );
};
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      {max && selected.length >= max &&
        <div style={{ color: theme.gold, fontSize: 11, marginTop: 5, fontWeight: 500 }}>Maksimal {max} ta</div>}
    </div>
  );
};
...
// Info tooltip modal
export const InfoModal = ({ icon, title, desc, onClose, theme }) => (
  <Modal title={`${icon} ${title}`} onClose={onClose} theme={theme}>
    <p style={{ fontSize: 14, color: theme.sub, lineHeight: 1.7, margin: 0 }}>{desc}</p>
    <div style={{ marginTop: 20 }}>
      <Btn onClick={onClose} theme={theme} v="primary" full>Tushunarli</Btn>
    </div>
  </Modal>
);
...
export const TrustBadge = ({ score, theme }) => {
  const [bg, col, icon] = score >= 80 ? [theme.successL, theme.success, "🛡"]
    : score >= 60 ? [theme.accentLight, theme.accent, "🛡"]
    : score >= 40 ? [theme.goldL, theme.gold, "⚡"]
    : [theme.card2, theme.hint, "⏳"];
  return <Badge bg={bg} color={col}>{icon} {score}</Badge>;
};
...
export const StatCard = ({ icon, value, label, theme, color, onClick, infoDesc }) => {
  const [showInfo, setShowInfo] = useState(false);
  return (
    <>
      <div
        onClick={onClick || (infoDesc ? () => setShowInfo(true) : undefined)}
        style={{
          background: theme.card, borderRadius: 16, padding: "14px 10px",
          textAlign: "center", border: `1px solid ${theme.border}`,
          cursor: (onClick || infoDesc) ? "pointer" : "default",
          position: "relative"
        }}>
        {infoDesc && (
          <div style={{ position: "absolute", top: 6, right: 8, fontSize: 14, color: theme.hint }}>ℹ</div>
        )}
      ...
      {showInfo && infoDesc && (
        <InfoModal icon={icon} title={label} desc={infoDesc} onClose={() => setShowInfo(false)} theme={theme} />
      )}
    </>
  );
};
